...
try:
    ...
    logger.warning("crimson_plot_helpers not available; using local implementations")
...
# Also available via osd_dump_parsers.parse_dump_filename.
...
# ---------------------------------------------------------------------------
# Plotting helpers (local fallbacks when crimson_plot_helpers is unavailable)
# ---------------------------------------------------------------------------
...
if not _HAS_PLOT_HELPERS:
    ...
    def _bucket_columns(df: pd.DataFrame) -> List[str]:  # type: ignore[misc]
    ...
    def _bucket_labels(bucket_cols: List[str]) -> List[str]:  # type: ignore[misc]
    ...
    def _save_or_show(fig: plt.Figure, outpath: Optional[str], gen_only: bool) -> None:  # type: ignore[misc]
        if outpath:
...
if not _HAS_PLOT_HELPERS:
    # ---------------------------------------------------------------------------
    # Local fallback implementations of all plot_* functions.
    # Used only when crimson_plot_helpers cannot be imported.
    # ---------------------------------------------------------------------------

    def plot_concurrent(  # type: ignore[misc]
        df: pd.DataFrame,
    ...
    def plot_stage_lat_heatmap(  # type: ignore[misc]
        df: pd.DataFrame,
        ...
        sns.heatmap(pivot, ax=ax, annot=True, fmt=".1f", cmap="YlOrRd",
                    linewidths=0.4, cbar_kws={"label": "mean latency (ms)"})
        ax.set_title(f"seastore_do_transaction_stage_lat – mean (ms) [tail={tail_filter}]")
        ax.set_xlabel("I/O Queue Depth")
        ax.set_ylabel("Stage")
        plt.tight_layout()
        _save_or_show(fig, outpath, gen_only)

    def plot_stage_lat_histogram(  # type: ignore[misc]
        df: pd.DataFrame,
        stage: str,
        tail: str = "all",
        outpath: Optional[str] = None,
    ...
    def plot_stage_lat_by_qd(  # type: ignore[misc]
        df: pd.DataFrame,
        ...
        """Line chart: mean latency per stage vs. QD."""
        if df.empty:
            return
        ...
        fig, axes = plt.subplots(len(stage_list), 1,
        ...
        for ax, stage in zip(axes[:, 0], stage_list):
            for tail in tails:
                ...
                if sub2.empty:
                    continue
                y = [sub2.loc[sub2["qd"] == qd, "mean_ms"].values[0]
                     if qd in sub2["qd"].values else np.nan for qd in qd_vals]
                ax.plot(xticks, y, label=tail,
                        linestyle=_TAIL_LINESTYLE.get(tail, "-"),
                        marker="o", markersize=5, color=color_map[tail])
        ...
        fig.suptitle("seastore_do_transaction_stage_lat – mean per stage vs. QD",
                     y=1.01, fontsize=11)
        plt.tight_layout()
        _save_or_show(fig, outpath, gen_only)

    def plot_conflict_histogram(  # type: ignore[misc]
        df: pd.DataFrame,
        outpath: Optional[str] = None,
        gen_only: bool = True,
    ) -> None:
        """Grouped bar chart of per-bucket conflict-replay counts across QDs."""
        if df.empty:
    ...
    def plot_conflict_mean_vs_qd(  # type: ignore[misc]
        df: pd.DataFrame,
        outpath: Optional[str] = None,
        gen_only: bool = True,
    ) -> None:
