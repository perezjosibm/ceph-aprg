...
class Job(object):
    ...
    RE_JOBS_COL = {
        ...
        'VIRT': { 'regex': re.compile(r"(\d+[.\d]*\w?)"), 'name': JOB_VIRT }, # memVirtual
        ...
        'TIME+': { 'regex': re.compile(r"(\d+:\d+[.\d]*)"), 'name': JOB_TIME }, # cpuTotalTime
    ...
    RE_DEFAULT_JOB_HEADER = " PID USER      PR  NI    VIRT    RES    SHR S  %CPU %MEM     TIME+ COMMAND"
    ...
    def parse(self, line:str):
        ...
        for col in self.columns:
            ...
            if col_match:
                ...
                logger.error("Failed to parse {0} from {1}".format(metric['name'], line))
