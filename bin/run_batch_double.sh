...
test_row["title"]='== 2 OSD 1 reactor default, FIO: unrestricted =='
test_row['fio']="0-31"
test_row['test']="crimson_2osd_default_fio_unrest"
string=$(declare -p test_row)
test_table["0"]=${string}

test_row["title"]='== 2 OSD 1 reactor default, FIO: 8 cores  =='
test_row['fio']="8-15"
test_row['test']="crimson_2osd_default_8fio"
string=$(declare -p test_row)
test_table["1"]=${string}

test_row["title"]='== 2 OSD 1 reactor default, FIO: 4 cores  =='
test_row['fio']="12-15"
test_row['test']="crimson_2osd_default_4fio"
string=$(declare -p test_row)
test_table["2"]=${string}

test_row["title"]='== 2 OSD 1 reactor default, FIO: 1 core  =='
test_row['fio']="15-15"
test_row['test']="crimson_2osd_default_1fio"
string=$(declare -p test_row)
test_table["3"]=${string}
#########################################
...
for KEY in "${!test_table[@]}"; do
  ...
  echo "MDS=0 MON=1 OSD=2 MGR=1 ../src/vstart.sh --new -x --localhost --without-dashboard --bluestore --redirect-output --bluestore-devs ${BLUESTORE_DEVS} --crimson --no-restart"
  MDS=0 MON=1 OSD=2 MGR=1 ../src/vstart.sh --new -x --localhost --without-dashboard --bluestore --redirect-output --bluestore-devs ${BLUESTORE_DEVS} --crimson --no-restart
