...
for KEY in "${!test_table[@]}"; do
  ...
  RBD_NAME=fio_test_0 RBD_SIZE="10G" fio ${FIO_JOBS}rbd_prefill.fio && rbd du fio_test_0 && /root/bin/run_fio.sh -s -w hockey -a -c "0-31" -f "${test_row["fio"]}" -p ${test_row["test"]} -k -n # w/o osd dump_metrics
  /ceph/src/stop.sh
  sleep 60
done
exit
