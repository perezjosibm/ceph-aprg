...
"""
    ...
    def gen_basic_csv_cmp(self):
        """
        Generate a basic comparison of the datasets loaded from the .csv files in the input_dirs.
        For each name in the input_dirs, plot the dataframe in the ds_list,
        using the same x and y columns, and generate a comparison chart. The x
        corresponds to the 'iops' column of the dataframe, the y column
    ...
    def load_files(self, input_dirs: Dict[str, Any]):
        ...
        def recreate_cpu_core_avg(
            name: str, workload: str, test_run: str, dir_path: str
        ):
            """
            ...
            if not os.path.isfile(pid_p):
                logger.error(f"PID file {pid_p} does not exist, bailing out")
                return (False, "")
        ...
        def recreate_bench(
            ...
            def _recreate_cpu_avg(name: str):
                ...
                if name not in cpu_filter:
                    logger.error(f"Invalid {name} -- assuming default 'cpu_cores'")
                    name = "cpu_cores"
