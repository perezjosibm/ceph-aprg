...
for NUM_OSD in 8; do
  for NUM_REACTORS in 4 8; do #1

    echo "== $NUM_OSD OSD crimson, $NUM_REACTORS reactor, fixed FIO 8 cores, latency target =="

    echo "MDS=0 MON=1 OSD=$NUM_OSD MGR=1 ../src/vstart.sh --new -x --localhost --without-dashboard --bluestore --redirect-output --bluestore-devs ${BLUESTORE_DEVS} --crimson --crimson-smp $NUM_REACTORS --no-restart"
    MDS=0 MON=1 OSD=$NUM_OSD MGR=1 ../src/vstart.sh --new -x --localhost --without-dashboard --bluestore --redirect-output --bluestore-devs "${BLUESTORE_DEVS}" --crimson --crimson-smp $NUM_REACTORS --no-restart

    test_name="crimson_${NUM_OSD}osd_${NUM_REACTORS}reactor_8fio_lt"
    ...
    RBD_NAME=fio_test_0 RBD_SIZE="10G" fio ${FIO_JOBS}rbd_prefill.fio && rbd du fio_test_0 && \
      /root/bin/run_fio.sh -s -w ex8osd -l -a -c "0-111" -f $FIO_CPU_CORES -p "$test_name" -n -k  # w/o osd dump_metrics
